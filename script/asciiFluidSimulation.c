    ...
    while (1)
    {
        ...
        for (particlesCursor = 0; particlesCursor < totalOfParticles; particlesCursor++)
        {
            ...
            for (particlesCursor2 = 0; particlesCursor2 < totalOfParticles; particlesCursor2++)
            {
                ...
                if (floor(1.0 - creal(particlesInteraction)) > 0)
                {
                    particles[PARTICLE particlesCursor _FORCE] += particlesInteraction * (particlesDistance * (3 - particles[PARTICLE particlesCursor _DENSITY] - particles[PARTICLE particlesCursor2 _DENSITY]) * pressure + particles[PARTICLE particlesCursor _VELOCITY] * viscosity - particles[PARTICLE particlesCursor2 _VELOCITY] * viscosity) / particles[PARTICLE particlesCursor _DENSITY];
                }
            }
        }
